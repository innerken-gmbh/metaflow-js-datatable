...
import { resetBaseUrl } from './baseUrlSetting'
...
export async function login (id, displayName,loginSuccess,loginFiled,loginEnd,code) {
  ...
  for (const storeId of allUserStoreId) {
    ...
    if (getStoreSub.includes(code)){
      usefulStoreId.push(storeId)
    }
    console.log(getStoreSub)
  }
  console.log(usefulStoreId, 'id')
  ...
  if (lastTimeLoginStore) {
    await resetBaseUrl(lastTimeLoginStore.deviceId,loginEnd)
  } else {
    if (usefulStoreId.length === 1) {
      ...
      await resetBaseUrl(usefulStoreId[0],loginEnd)
